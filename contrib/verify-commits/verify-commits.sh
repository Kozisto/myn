...
VERIFIED_ROOT=$(cat "${DIR}/trusted-mynt-cli-root")
...
while true; do
	...
	if ! mynt-cli -c "gpg.program=${DIR}/gpg.sh" verify-commit "$CURRENT_COMMIT" > /dev/null; then
		if [ "$PREV_COMMIT" != "" ]; then
			echo "No parent of $PREV_COMMIT was signed with a trusted key!" > /dev/stderr
			echo "Parents are:" > /dev/stderr
			PARENTS=$(mynt-cli show -s --format=format:%P $PREV_COMMIT)
			for PARENT in $PARENTS; do
				mynt-cli show -s $PARENT > /dev/stderr
			done
		else
	...
	if [ "$VERIFY_TREE" = 1 -o "$PREV_COMMIT" = "" ]; then
		...
		IFS='
'
		for LINE in $(mynt-cli ls-tree --full-tree -r "$CURRENT_COMMIT"); do
			case "$LINE" in
				"12"*)
		...
		FILE_HASHES=""
		for FILE in $(mynt-cli ls-tree --full-tree -r --name-only "$CURRENT_COMMIT" | LC_ALL=C sort); do
			if [ "$HAVE_GNU_SHA512" = 1 ]; then
				HASH=$(mynt-cli cat-file blob "$CURRENT_COMMIT":"$FILE" | sha512sum | { read FIRST _; echo $FIRST; } )
			else
				HASH=$(mynt-cli cat-file blob "$CURRENT_COMMIT":"$FILE" | shasum -a 512 | { read FIRST _; echo $FIRST; } )
			fi
...
'
		...
		HASH_MATCHES=0
		MSG="$(mynt-cli show -s --format=format:%B "$CURRENT_COMMIT" | tail -n1)"
	...
	PARENTS=$(mynt-cli show -s --format=format:%P "$CURRENT_COMMIT")
	for PARENT in $PARENTS; do
