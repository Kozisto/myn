...
# Taken from mynt-cli-subtree (Copyright (C) 2009 Avery Pennarun <apenwarr@gmail.com>)
find_latest_squash()
{
	dir="$1"
	sq=
	main=
	sub=
	mynt-cli log --grep="^mynt-cli-subtree-dir: $dir/*\$" \
		--pretty=format:'START %H%n%s%n%n%b%nEND%n' "$COMMIT" |
	while read a b _; do
		case "$a" in
			START) sq="$b" ;;
			mynt-cli-subtree-mainline:) main="$b" ;;
			mynt-cli-subtree-split:) sub="$b" ;;
			END)
				if [ -n "$sub" ]; then
					if [ -n "$main" ]; then
						# a rejoin commit?
...
tree_actual=$(mynt-cli ls-tree -d "$COMMIT" "$DIR" | head -n 1)
...
tree_commit=$(mynt-cli show -s --format="%T" $old)
...
if [ "$tree_actual_tree" != "$tree_commit" ]; then
    mynt-cli diff $tree_commit $tree_actual_tree >&2
    echo "FAIL: subtree directory was touched without subtree merge" >&2
    exit 1
fi

# get the tree in the subtree commit referred to
if [ "d$(mynt-cli cat-file -t $rev 2>/dev/null)" != dcommit ]; then
    echo "subtree commit $rev unavailable: cannot compare" >&2
    exit
fi
tree_subtree=$(mynt-cli show -s --format="%T" $rev)
