...
--no-commit     Do not commit anything to mynt-cli
...
if [[ $setup = true ]]
then
    sudo apt-get install ruby apache2 mynt-cli apt-cacher-ng python-vm-builder qemu-kvm qemu-utils
    mynt-cli clone https://github.com/bitcoin-core/gitian.sigs.mynt-cli
    mynt-cli clone https://github.com/bitcoin-core/bitcoin-detached-sigs.mynt-cli
    mynt-cli clone https://github.com/devrandom/gitian-builder.mynt-cli
...
then
	...
	popd
	    ...
	    # Commit to gitian.sigs repo
            ...
            echo ""
            pushd gitian.sigs
            mynt-cli add ${VERSION}-linux/${SIGNER}
            mynt-cli add ${VERSION}-win-unsigned/${SIGNER}
            mynt-cli add ${VERSION}-osx-unsigned/${SIGNER}
            mynt-cli commit -a -m "Add ${VERSION} unsigned sigs for ${SIGNER}"
            popd
        fi
fi
...
then
	...
	popd
        ...
        then
            ...
            echo ""
            mynt-cli add ${VERSION}-win-signed/${SIGNER}
            mynt-cli add ${VERSION}-osx-signed/${SIGNER}
            mynt-cli commit -a -m "Add ${VERSION} signed binary sigs for ${SIGNER}"
            popd
        fi
fi
