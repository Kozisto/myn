Mynt Core version 2.13.3 is now available from:
...
entire blockchain after an unclean shutdown of Mynt Core on Windows
...
Previous versions of Mynt Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. 

Mynt Core 2.13.3 will have a strict maximum size on the mempool. The
...
Mynt Core 2.13.3 also introduces new default policy limits on the length and
...
Mynt Core 2.13.3 nodes. Mynt Core will only allow replacement of
...
Note that the wallet in Mynt Core 2.13.3 does not yet have support for
...
This allows running myntd without having to do any manual configuration.
...
Mynt Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Mynt Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Mynt Core 2.13.3, when mempool limit has been reached a higher minimum
...
Mynt Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Mynt Core automatically creates a hidden service to listen on, without
manual configuration. Mynt Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Mynt Core is listening, and
...
myntd can now (optionally) asynchronously notify clients through a
...
Core to use floating fees. By default, Mynt Core will use floating
fees.
...
(default: `0.0002` MYNT/kB).

At all times, Mynt Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MYNT.
Furthermore, Mynt Core will never create transactions paying less than
...
caching. A sample config for apache2 could look like:
    ...
    <Location /groestlcoinrpc>
        ...
        # optional bypass myntd rpc basic auth
...
Restarting myntd will no longer clear out the list of banned peers; instead
...
Mynt Core 2.13.3 introduces mempool-only locktime
...
For example:
    ...
    $ src/mynt-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Mynt Core code base has
...
HD wallets are incompatible with older versions of Mynt Core.
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  note that Mynt Core does not provide an option to use this capability as
