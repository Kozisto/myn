Mynt Core version 2.11.0 is now available from:
...
backwards-compatible with older versions of Mynt Core or other software:
...
Block pruning allows Mynt Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Mynt Core will 
...
myntd makes it easier for experienced open source developers to contribute 
to the project.
...
Another change in the 2.11.0 release is moving away from the myntd executable
functioning both as a server and as a RPC client. The RPC client functionality
("tell the running groestlcoin daemon to do THIS") was split into a separate
executable, 'mynt-cli'. The RPC client code will eventually be removed from
myntd, but will be kept for backwards compatibility for a release or two.
...
Mynt Core now uses 'headers-first synchronization'. This means that we first
...
Starting from 2.11.0, the Mynt Core distribution includes a consensus library.
...
standard Mynt Core nodes wouldn't relay them to miners, nor would
most miners include them in blocks they mined.

mynt-tx
----------

It has been observed that many of the RPC functions offered by myntd are
"pure functions", and operate independently of the myntd wallet. This
included many of the RPC "raw transaction" API functions, such as
createrawtransaction.

mynt-tx is a newly introduced command line utility designed to enable easy
manipulation of groestlcoin transactions. A summary of its operation may be
obtained via "mynt-tx --help" Transactions may be created or signed in a
...
Mynt Core's block templates are now for version 3 blocks only, and any mining
...
Mynt Core now supports BIP 22 long polling, so mining software can be
...
Mynt Core 2.11.0 implements BIP 66, which introduces block version 3, and a new
