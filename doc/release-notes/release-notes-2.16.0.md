Mynt Core version *2.16.0* is now available from:
...
Mynt Core has supported automatically pruning the blockchain since 2.11.0 Pruning
...
When Mynt Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `mynt-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`mynt-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/mynt-cli -named help command="help"
    src/mynt-cli -named getblockhash height=0
    src/mynt-cli -named getblock blockhash=0000000000000036ae5aabb18a2d345d19d60ba1927f071af2aa8eec7a27d41d
    src/mynt-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
(in Mynt Core 2.1.0.6) it has been disabled by default.
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If Mynt Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
...
Mynt Core 2.16.0 contains the following changes to the RPC interface and `mynt-cli` for multi-wallet:

* When running Mynt Core with a single wallet, there are **no** changes to the RPC interface or `mynt-cli`. All RPC calls and `mynt-cli` commands continue to work as before.
* When running Mynt Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>` endpoint, and `mynt-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Mynt Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>` endpoint, for example `127.0.0.1:1441/wallet/wallet1.dat`. `mynt-cli` commands should be run with a `-rpcwallet` option, for example `mynt-cli -rpcwallet=wallet1.dat getbalance`.
...
Mynt Core has supported creating opt-in replace-by-fee (RBF) transactions
...
In previous versions of Mynt Core, a portion of each block could be reserved for transactions based on the age and value of UTXOs they spent. This concept (Coin Age Priority) is a policy choice by miners, and there are no consensus rules around the inclusion of Coin Age Priority transactions in blocks. In practice, only a few miners continue to use Coin Age Priority for transaction selection in blocks. Mynt Core 2.16 removes all remaining support for Coin Age Priority (See [PR 9602](https://github.com/bitcoin/bitcoin/pull/9602)). This has the following implications:
...
A number of changes to the way Mynt Core deals with peer connections and invalid blocks
...
Mynt Core now has more flexibility in where the wallets directory can be
...
------------------------------------
The minimum version of the GCC compiler required to compile Mynt Core is now 4.8. No effort will be
...
- `uptime` returns the total runtime of the `myntd` server since its last start (See [PR 10400](https://github.com/bitcoin/bitcoin/pull/10400)).

- When using Mynt Core in multi-wallet mode, RPC requests for wallet methods must specify
...
- The second argument in `prioritisetransaction` has been renamed from `priority_delta` to `dummy` since Mynt Core no longer has a concept of coin age priority. The `dummy` argument has no functional effect, but is retained for positional argument compatibility. See [Removal of Coin Age Priority](#removal-of-coin-age-priority).
...
- mynt-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- mynt-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
